...
DEFAULT_REPORT_PROMPT_PATH = "prompt.md"
DEFAULT_REPOSITORY_ROOT_PLACEHOLDER = "{{REPOSITORY_ROOT}}"


def build_repository_gui_evaluation_prompt(repository_root: str | None = None) -> str:
    target_root = repository_root or DEFAULT_REPOSITORY_ROOT_PLACEHOLDER
    return f"""# Repository GUI Evaluation Report Prompt

## Mission

Interrogate the repository at `{target_root}` and extract how it uses PyQt6, including architecture, styling, layout systems, widget inventories, interactions, dialogs, menus, graphics scenes, custom painting, model-view patterns, threading, packaging, and any distinctive implementation patterns.
...
def write_repository_gui_evaluation_prompt(
    output_path: str | Path = DEFAULT_REPORT_PROMPT_PATH,
    *,
    repository_root: str | None = None,
) -> Path:
    path = Path(output_path)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(build_repository_gui_evaluation_prompt(repository_root), encoding="utf-8")
    return path